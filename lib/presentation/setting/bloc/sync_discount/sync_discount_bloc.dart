...
class SyncDiscountBloc extends Bloc<SyncDiscountEvent, SyncDiscountState> {
  ...
  SyncDiscountBloc(this._discountRemoteDatasource, this._local) : super(const SyncDiscountState.initial()) {
              ...
              try {
                ...
                if (action == 'create') {
                  final req = DiscountRequestModel(
                    ...
                    combinable: payload['combinable'] == true || payload['combinable'] == 1,
                    status: payload['status'] ?? 'active',
                    startDate: payload['start_date'],
                    expiredDate: payload['expired_date'],
                    startTime: payload['start_time'],
                    endTime: payload['end_time'],
                    validDays: (payload['valid_days'] as List?)?.map((e) => e as int).toList(),
                  );
                  final created = await _discountRemoteDatasource.createDiscount(req);
